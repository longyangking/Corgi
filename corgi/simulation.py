...
if __name__=='__main__':
    ...
    print('Inner test: Sim')
    y0, t0 = [1.0j, 2.0], 0
    def fun(t,y):
        return [1j*2*y[0] + y[1], -2*y[1]**2]
    sim = simulation(fun=fun,y0=y0,t0=t0)
    print('Sim status: ',sim.init())

    # Test fastsimulation class
    print('Inner test: Fast Sim')
    def fun(y, t):
        theta, omega = y
    ...
    sim = fastsimulation(fun=fun,y0=y0)
    print('Fast Sim status: ',sim.init())
    sol = sim.evaluate(ts)
